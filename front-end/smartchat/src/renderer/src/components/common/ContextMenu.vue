  ...
  >
    <ul>
      <li
        ...
        :class="{ 'border-top': contextMenu.borderTop }"
        @click.stop="handleMenuClick(contextMenu.type)"
      >
        {{ contextEnums[contextMenu.type] }}
      </li>
    </ul>
  </div>
  <!-- <DragModal /> -->
...
watch(
  ...
  () => {
    ...
    if (showInModule === 'chat') {
      ...
      if (carryEntryInfo.unReadCount) {
        contextMenus[showInModule][1]['type'] = 'hasRead'
      } else {
        contextMenus[showInModule][1]['type'] = 'unRead'
      }
    }
