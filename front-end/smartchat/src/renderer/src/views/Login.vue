...
client.notificationService.addNotificationListener((notification) => {
  console.info(
    `onNotification: Receive a notification from other users or server: ${JSON.stringify(notification)}`
  )
})
...
client.messageService.addMessageListener((message) => {
  console.info(
    `onMessage: Receive a message from other users or server: ${JSON.stringify(message)}`
  )
})
